...
print(
    f"Точка принадлежит гиперболическому параболоиду: {'да' if point_on_paraboloid else 'нет'}"
)
print(
    f"Плоскость пересекает гиперболический параболоид: {'да' if intersection_plane else 'нет'}"
)
print(
    f"Прямая пересекает гиперболический параболоид: {'да' if intersection_line else 'нет'}"
)
